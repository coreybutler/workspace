  ...
  choices: function () {
    ...
    return new Promise((resolve, reject) => {
      req.get('https://raw.githubusercontent.com/docker-library/official-images/master/library/node', (res) => {
        res.on('data', (data) => raw += data).on('end', function () {
          raw = raw.split('\n').filter((line) => {
          ...
          raw.push({
            name: 'Other Docker Image',
            value: 'other'
          })

          resolve(choices.concat(raw))
        })
      })
    })
  }
...
module.exports = (preferences) => {
  ...
  inquirer.prompt(question).then((answers) => {
    ...
    tasks.add((next) => {
      ...
      if (Object.keys(docker_env).filter((attr) => attr.trim().length > 0).length === 0) {
        docker_env = {}
      }

      if (Object.keys(local_env).filter((attr) => attr.trim().length > 0).length === 0) {
        local_env = {}
      }
